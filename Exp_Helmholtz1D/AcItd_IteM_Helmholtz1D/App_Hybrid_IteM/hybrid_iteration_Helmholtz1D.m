function [uh_Hybrid, eh_Hybrid, err_frq_Hybrid, eh_L2_Hybrid, rh_L2_Hybrid] =hybrid_iteration_Helmholtz1D(Ah,fh,uh,eigVec_B,NN_Preconditioner,L,K,omega)

format short e

% Input:
% Ah, fh, uh, eigVec_B : Discrete system matrix, right-hand side, 
%                        exact solution, and eigenvectors of iterator B
...
for l=1:K*L
    if mod(l,K)==0
        ...
        e_repNG_MeshPts = h * NN_Preconditioner* Rh;
...
for l =  1 : number_of_iteration             
    err_frq_Hybrid(:, l) = abs(sum(repmat( eh_Hybrid(:,l), 1, matrix_size ) .*  eigVec_B .* h))'; 
