...
def SmpPts_Bndry_Line1D(num_pts_y): 
    ...
    BndryPts_y = torch.rand(num_pts_y,1)
    BndryPts_x_Reshape = BndryPts_x.repeat(num_pts_y,1)
    BndryPts_y_Reshape = torch.reshape(BndryPts_y.repeat(1,2), (2*num_pts_y,1))
    
    return torch.cat((BndryPts_x_Reshape,BndryPts_y_Reshape),1)
