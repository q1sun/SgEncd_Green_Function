...
class TraindataInterior(Dataset):    
    def __init__(self, num_reglr_pts_x): 
        
        # SmpPts_Train[ x-SmpPt, y-SmpPt, 0, 1 ], each x-SmpPt aligns with all y-SmpPts inside (0,1)
        self.SmpPts_Train = Sample_Points.SmpPts_Reglr_Line1D(num_reglr_pts_x)                        
...
# create training and testing datasets         
traindata_reglr = TraindataInterior(num_reglr_pts_y)
traindata_bndry = TraindataBoundary(num_reglr_pts_y)
...
fig = plt.figure()
plt.scatter(traindata_reglr.SmpPts_Train[:,0], traindata_reglr.SmpPts_Train[:,1], c = 'red', label = 'interior points' )
...
def train_epoch(epoch, model, optimizer, device):
    ...
    training_loss_epoch, loss_reglr_epoch, loss_bndry_epoch, loss_snglr_epoch, loss_symtry_epoch = 0, 0, 0, 0, 0
    ...
    for _, ( data_reglr, data_bndry, data_snglr ) in \
        ...
        smppts_symtry_aug = torch.cat( (smppts_reglr[:,[1]], smppts_reglr[:,[0]], Exact_Solution.augmented_variable(smppts_reglr[:,[1]], smppts_reglr[:,[0]])), 1 )
        ...
        Laplacian_G_reglr = (Gxx_NN_reglr + Gzz_NN_reglr + 2 * (Gxz_NN_reglr * (smppts_reglr[:,[0]] > smppts_reglr[:,[1]]) - Gxz_NN_reglr * (smppts_reglr[:,[0]] < smppts_reglr[:,[1]])))

        # construct mini-batch loss function and then perform backward pass
        loss_reglr = torch.mean( torch.pow( Laplacian_G_reglr, 2 ) )
        loss_snglr = torch.mean( torch.pow( 2 * Gz_NN_snglr + 1, 2 ) )
...
def test_epoch(epoch, model, optimizer, device):
    ...
    for smppts_test, G_exact_smppts in dataloader_test:
        ...
        NN_smppts = model(torch.cat((smppts_test, Exact_Solution.augmented_variable(smppts_test[:,[0]],smppts_test[:,[1]])), 1)) 
        
        loss_epoch = torch.mean( torch.pow( torch.squeeze(NN_smppts) - G_exact_smppts, 2 ) )                 
                
        # integrate loss      
        testing_loss_epoch += loss_epoch.item()          
    
    return testing_loss_epoch
##############################################################################################
...
with torch.no_grad(): 
    ...
    plt.title('Predicted Green Function on Test Dataset')
