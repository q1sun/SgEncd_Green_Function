...
grid_data = h5py.File('Data/mesh_Gpts_h{0}.mat'.format(number_of_elements))

SmpPts_Gpts = np.float64(grid_data['mesh_Gpts']) # data type: float64 
SmpPts_Gpts = torch.from_numpy(SmpPts_Gpts).t()

SmpPts_Gpts_Symtry = np.float64(grid_data['mesh_Gpts_symtry']) # data type: float64 
SmpPts_Gpts_Symtry = torch.from_numpy(SmpPts_Gpts_Symtry).t()

# compute NN prediction of Green's function
with torch.no_grad():    
